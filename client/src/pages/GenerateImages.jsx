  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      <form className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200' onSubmit={onSubmitHandler}>
        ...
        <textarea onChange={(e) => setInput(e.target.value)} value={input} rows={4} className='border w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border-gray-300' placeholder='Describe what you want to see in image...' required />
        ...
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11 cursor-pointer'>
          {imageStyle.map((item) => (
            <span onClick={() => setSelectedStyle(item)}
              className={`text-xs px-4 py-1 border rounded-full ${selectedStyle === item ? "bg-green-50 text-green-700" : "text-gray-500 border-gray-300"}`} key={item} >
              {item}
            </span>
          ))}
        </div>
